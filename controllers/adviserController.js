...
const getAdviserProfile = asyncHandler(async (req, res) => {
  ...
  console.log(`Adviser profile fetched for ${userId}`);
  res.status(200).json({
    success: true,
    data: results[0]
  });
...
const getAdviserSubjects = asyncHandler(async (req, res) => {
  const userId = req.user.user_id;
...
const getTeachingGradingPeriods = asyncHandler(async (req, res) => {
...
const getAdvisorySection = asyncHandler(async (req, res) => {
  const userId = req.user.user_id;
...
const getAdvisoryStudentList = asyncHandler(async (req, res) => {
  const userId = req.user.user_id;
  const { school_year } = req.query;
  ...
  const [sectionResults] = await db.query(sectionSql, params);
  ...
  const [studentResults] = await db.query(studentSql, [sectionId]);
...
const getAdvisoryStudentGrades = asyncHandler(async (req, res) => {
  const userId = req.user.user_id;
  ...
  const [sectionResults] = await db.query(sectionSql, [userId, school_year]);
  ...
  const [results] = await db.query(gradesSql, [grading_period, school_year, grading_period, school_year, sectionId]);
...
const getAdvisoryAchievements = asyncHandler(async (req, res) => {
  const userId = req.user.user_id;
  const { grading_period, school_year } = req.query;
  ...
  const [results] = await db.query(achievementSql, params);
...
const getGradingPeriods = asyncHandler(async (req, res) => {
  ...
  const [sectionResults] = await db.query(sectionSql, [userId]);
